...
use crate::traits::backend::{ChatTurn, LLMBackend, PromptFlavor};
use crate::traits::sampling::SamplingParams as CoreSamplingParams;
...
impl LlamaBackendImpl {
    /// Create default llama.cpp params, with a few env overrides for dev convenience.
    ...
    /// Construct a new session (context) from an existing model + params.
    pub fn from_model(model: Arc<LlamaModel>, params: LlamaParams) -> Result<Self, String> {
        // SAFETY: lifetime widen is OK: model & context live inside the same struct and drop in order.
        ...
        let context = static_ref
            .create_context(ctx_params, /*embeddings_enabled*/ false)
    ...
    /// Spawn a fresh session (new context) reusing the same loaded model.
...
impl LLMBackend for LlamaBackendImpl {
    fn load<P: AsRef<Path>>(model_path: P) -> Result<Self, String> {
        ...
        // Load weights via llama.cpp wrapper
        let backend =
            LlamaCppBackend::load(&model_path, params.clone()).map_err(|e| format!("{e}"))?;
        let model = backend.model();

        // Create the initial context for this session
        // (see safety note in `from_model`)
    ...
    fn detokenize_range(
        ...
        let toks: Vec<llama_rs::token::LlamaToken> = slice
            .iter()
            .map(|crate::traits::token::Token(t)| llama_rs::token::LlamaToken(*t))
            .collect();

        // Uses the fixed function above; returns empty Vec when there's nothing to render.
        self.context
