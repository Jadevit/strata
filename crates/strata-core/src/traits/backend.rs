...
/// A single chat turn (role + content). If a backend exposes
/// a native chat template, it can consume a sequence of these
/// and return a fully-rendered prompt.
#[derive(Clone, Debug)]
pub struct ChatTurn {
    pub role: Role,
    pub content: String,
...
/// - If your backend supports cheap session cloning (e.g., new context from resident weights),
///   feel free to add a `spawn()` on the concrete type—do not put it in this trait.
